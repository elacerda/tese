...
Essa tese tem como cerne o artigo sobre o DIG nas regiões de galáxias de todos os tipos de Hubble presentes no CALIFA  \citep[][Apêndice \ref{apendice:DIGpaper0}]{Lacerda.etal.2018}, sobre o qual trabalhei nos dois últimos anos sob orientação do Prof. Dr. Roberto Cid Fernandes Jr \edu{\ojo e coorientação da Natalia?}. Nele separamos regiões com o regime de ionização dominado por formação estelar (SF) daquelas com gás ionizado difuso (DIG). Anteriormente a esse trabalho, porém durante o doutorado, trabalhei em fragmentos de diversos artigos, além da construção de um módulo que possibilitou unir os resultados da síntese com as medidas nebulares, parte técnica fundamental para o estudo principal desta tese. No Capítulo \ref{sec:sample} descrevo a amostra de galáxias utilizadas neste trabalho, minha participação no artigo de \citet[][Apêndice \ref{apendice:GBetal2015a}]{GarciaBenito.etal.2015a} e a construção da classe EmLinesDataCube. O artigo sobre o DIG será dividido em dois capítulos. Nele fiz toda a parte de programação, figuras e participei de todas as discussões e análises envolvendo o tema, além de apresentá-lo em duas conferências. A exposição dos argumentos teóricos e empíricos no qual se embasam o nosso método de classificação aparece no Capítulo \ref{sec:DIGclass}, e no próximo (Capítulo \ref{sec:DIGdisc}) discutimos diversos temas acerca da aplicação de nossa classificação nas zonas de galáxias do CALIFA. No capítulo \ref{sec:concl} Por fim, demais artigos e trabalhos são apresentados no Cap.\ \ref{sec:more}.
